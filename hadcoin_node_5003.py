...
class Blockchain:
    ...
    def __init__(self):
        self.chain=[]     #Contains all the block in a chain
        self.trasactions=[];
        self.create_block(proof=1,previous_hash='0')
        self.nodes=set()
        
    #Creates a block and empty the transaction array
    ...
    #proof of work is done to produce the desired hash
    ...
    #Hashes a block and returns its hash
    def hash(self,block):
    ...
    def is_chain_valid(self,chain):
        ...
        block_index=1
        while block_index < len(chain):  #Iterate through all the blocks in the chain
            ...
            if hash_operation[:4]!='0000':
                return False
            previous_block=block  #Hash the current block with the help of previous proof and check whether its the required hash
            block_index+=1
        return True
    
    #Add the transaction to the transaction array and returns the index of the block in which its going to be included
    ...
    #Adds the nodes to node variable
    def addNode(self,address):
        parsed_url=urlparse(address)
        self.nodes.add(parsed_url.netloc)

#Checks the chain in all the nodes, if chain is less than any of the chain in the node then replaces the current chain with that
...
def mine_block():
        ...
        blockchain.addTrasanction(sender=node_address,receiver='Alok',amount=10)
